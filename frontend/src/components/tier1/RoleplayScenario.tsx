...
function excerptToBullets(text: string): string[] {
    ...
    return bullets.length > 0
        ? bullets.slice(0, 4)
        : ['Review the cited manual section for the exact procedure and key safety steps.'];
}

function flattenParts(scenarios: ScenarioScript[], selectedScenarioId: string) {
    const filtered = scenarios.filter((item) => item.id === selectedScenarioId);
    const selectedScenarios = filtered.length > 0 ? filtered : scenarios.length > 0 ? [scenarios[0]] : [];
    ...
    return {
        scenario: selectedScenarios[0] ?? scenarios[0],
        parts,
...
export default function RoleplayScenario() {
    ...
    if (!scenario || totalParts === 0) {
        return (
            <Card className="roleplay-shell">
                <CardHeader>
                    <Badge variant="warning">No scenarios</Badge>
                    <CardTitle>Roleplay unavailable</CardTitle>
                </CardHeader>
                <CardContent>
                    <p className="muted-text">No roleplay scenarios are available right now. Check the generated scenario data and try again.</p>
                </CardContent>
            </Card>
        );
    }
        ...
        return (
            <Card className="roleplay-shell">
                ...
                <CardFooter className="actions-row">
                    <Button variant="secondary" onClick={restart}>Retry same scenario</Button>
        ...
        <Card className="roleplay-shell">
            <CardHeader>
                <div className="roleplay-topline">
                    ...
                    <Badge variant="neutral">Officer mode</Badge>
                </div>

                <CardTitle>{displayScenario.name}</CardTitle>
            ...
            <CardContent className="roleplay-grid">
                <div className="scene-column">
                    ...
                    <div className="chat-panel" aria-label="Scenario description">
                        ...
                        <p className="muted-text">
                            {displayScenario.scenarioDescription ?? 'Use the prompt and manual citation to understand the scene before answering.'}
                        </p>
